  ...
  <div class="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans">
    ...
    <header class="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center shadow-sm">
      <div class="flex items-center gap-3">
        ...
        <span class="px-2.5 py-0.5 bg-slate-100 text-slate-500 rounded text-xs font-bold uppercase tracking-wider">Console</span>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="relative" ref="userMenuRef">
          <button
            ...
            class="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-slate-100 transition-colors duration-150 group"
            id="workspace-user-menu-btn"
          >
            <img :src="userAvatar" class="w-8 h-8 rounded-full border border-gray-200 shadow-sm" />
            <span class="text-sm font-semibold text-slate-700 group-hover:text-slate-900">{{ userFullName }}</span>
          ...
          <div
            v-if="showUserMenu"
            class="absolute right-0 top-full mt-2 w-52 bg-white border border-gray-200 rounded-xl shadow-lg z-50 py-1 overflow-hidden"
          >
            <div class="px-4 py-3 border-b border-gray-100">
              <p class="text-xs font-bold text-slate-800 truncate">{{ userFullName }}</p>
            ...
            <router-link
              ...
              class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-zyra-primary transition-colors w-full"
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex flex-col h-[75vh]">
        <div class="flex justify-between items-center mb-5 pb-3 border-b border-gray-100">
          <h2 class="text-lg font-bold text-slate-800">My Workspaces</h2>
        ...
        <div class="space-y-2 overflow-y-auto flex-grow pr-1">
          <div
            ...
            :key="w.id"
            :class="{ 'border-zyra-primary bg-orange-50/50': authStore.currentWorkspace?.id === w.id }"
            class="flex justify-between items-center p-3 border border-gray-200 rounded-lg hover:border-zyra-primary hover:bg-orange-50/20 transition duration-150 group"
          >
            <!-- Clickable workspace info -->
            <div class="flex-grow cursor-pointer" @click="selectWorkspace(w)">
              <p class="font-bold text-sm" :class="authStore.currentWorkspace?.id === w.id ? 'text-zyra-primary' : 'text-slate-800'">{{ w.name }}</p>
              <p class="text-[10px] text-slate-400">slug: {{ w.slug }}</p>
            </div>
            <!-- Role badge + Settings gear -->
            <div class="flex items-center gap-2">
              <span class="px-2 py-0.5 text-[9px] font-extrabold rounded uppercase bg-slate-100 text-slate-500">
                {{ w.role }}
              </span>
              <router-link
        ...
        <div v-if="authStore.currentWorkspace" class="mt-4 pt-4 border-t border-gray-100 space-y-2">
          <router-link
            to="/workspace-settings"
            class="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-xs font-semibold text-slate-600 hover:bg-orange-50 hover:text-zyra-primary border border-transparent hover:border-orange-200 transition"
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:col-span-2 flex flex-col h-[75vh]">
        <div class="flex justify-between items-center mb-5 pb-3 border-b border-gray-100">
          <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 overflow-y-auto flex-grow pr-1">
          ...
          <div
            ...
            :key="p.id"
            @click="openProject(p)"
            class="p-4 border border-gray-200 rounded-xl hover:border-zyra-primary hover:shadow-md cursor-pointer transition duration-200 flex flex-col justify-between bg-white group"
          >
            <div>
              <div class="flex justify-between items-start mb-2">
                <span class="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold tracking-wider group-hover:bg-orange-50 group-hover:text-zyra-primary">
                  {{ p.key }}
                </span>
                <span class="text-[10px] text-slate-400">Owner Lead</span>
              </div>
              <h3 class="font-bold text-base text-slate-800 group-hover:text-zyra-primary transition-colors">
                {{ p.name }}
              </h3>
              <p class="text-xs text-slate-500 mt-1 line-clamp-2">{{ p.description || 'No description provided.' }}</p>
            </div>
            
            <div class="mt-4 pt-3 border-t border-gray-100 flex justify-between items-center text-xs text-slate-400">
    ...
    <AppDialog v-model="showNewWorkspace" title="Create New Workspace" size="sm">
      <form @submit.prevent="createWorkspace" class="space-y-4">
        <div>
          ...
          <input type="text" v-model="newWorkspaceName" required placeholder="e.g. Acme Team" maxlength="50" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-zyra-primary outline-none" />
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-600 mb-1">Workspace Slug</label>
          <input type="text" v-model="newWorkspaceSlug" required placeholder="e.g. acme-team" maxlength="30" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-zyra-primary outline-none" />
        </div>
      </form>
      ...
      <form @submit.prevent="createProject" class="space-y-4">
        <div class="grid grid-cols-3 gap-3">
          <div class="col-span-2">
            <label class="block text-xs font-bold text-slate-600 mb-1">Name</label>
            <input type="text" v-model="newProjectName" required placeholder="Phoenix System" maxlength="80" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-zyra-primary outline-none" />
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-600 mb-1">Key (Upper)</label>
            <input type="text" v-model="newProjectKey" required placeholder="PHX" maxlength="5" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-zyra-primary outline-none uppercase" />
          </div>
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-600 mb-1">Description (Optional)</label>
          <textarea v-model="newProjectDesc" rows="2" placeholder="Describe the project scope..." maxlength="500" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-zyra-primary outline-none"></textarea>
        </div>
      </form>
      <template #footer="{ close }">
        <button type="button" @click="close" class="px-3 py-1.5 text-xs font-bold text-slate-400 hover:text-slate-600">Cancel</button>
