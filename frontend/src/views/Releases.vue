    ...
    <div v-if="showHelp" class="mb-6 p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-orange-50/80 via-amber-50/50 to-transparent dark:from-orange-950/20 dark:via-slate-800/40 dark:to-transparent border border-orange-200/50 dark:border-orange-500/10 shadow-sm backdrop-blur-sm relative transition duration-300">
      ...
      <div class="flex items-start gap-3.5">
        ...
        <div class="flex-1 min-w-0 pr-4">
          <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">📦 Release Cycles & Versioning</h3>
          <p class="text-xs text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">
            Releases and versioning allow you to track product builds, software version releases (like v1.0.0), and active deployment cycles.
          </p>
          <ul class="text-xs text-slate-500 dark:text-slate-400 mt-2 space-y-1.5 pl-4 list-disc">
            <li><strong>Planning Versions</strong>: Define release targets by creating versions with scheduled release dates.</li>
            <li><strong>Tracking Statuses</strong>: Shift version status through Planned, In Progress, Released, and Archived to keep the team aligned.</li>
          </ul>
        </div>
      </div>
    </div>
...
import { Pencil as PencilIcon, Trash2 as TrashIcon, Package as PackageIcon, AlertTriangle as AlertTriangleIcon, HelpCircle as HelpCircleIcon, X as XIcon } from 'lucide-vue-next';
import SelectDropdown from '../components/SelectDropdown.vue';

export default defineComponent({
  name: 'Releases',
  components: { PencilIcon, TrashIcon, PackageIcon, AlertTriangleIcon, SelectDropdown, HelpCircleIcon, XIcon },
  setup() {
    ...
    return { releases, loading, showCreateModal, editingRelease, form, saveRelease, editRelease, deleteRelease, closeModal, statusBadgeClass, formatDate, statusOptions, showDeleteConfirm, deleteTarget, confirmDelete, showHelp };
  },
});
</script>
